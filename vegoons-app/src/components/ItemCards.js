...
import {Card, Image, Flex, Text, Box, Animation} from '@fluentui/react-northstar'
...
const RestaurantCard = ({info, delay}) => {
  const ratingRound = rating => Math.round(rating*2)/2
  
  return (
    <a href={info.url} target='_blank' rel='noreferrer'>
      <Animation name='slideUp' delay={`${delay * 0.2}s`} duration="0.2s">
        <Card elevated styles={{opacity: 0}}>
          <Card.Header>
            ...
            <Flex gap="gap.medium">
              ...
              <Flex.Item grow>
                <Flex column gap='gap.smaller'>
                  ...
                  <Text weight='bold'>{info.transactions?.join('/')}</Text>
                </Flex>
              </Flex.Item>
            </Flex>
          </Card.Body>
